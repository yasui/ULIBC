...
static inline void SET_BITWRD(bitmap_t *wrd, int v) {
  *wrd |=  (1ULL << v);
}
static inline void UNSET_BITWRD(bitmap_t *wrd, int v) {
  *wrd &= ~(1ULL << v);
}
static inline int ISSET_BITWRD(bitmap_t *wrd, int v) {
  return ( *wrd & (1ULL << v) ) != 0;
}
...
static inline int bit_iter(bitmap_t *bits) {
  const int i = cnttz_uint64(*bits);
  UNSET_BITWRD(bits,i);
  return i;
}
...
static inline uint64_t fetch_and_or_uint64(uint64_t *p, uint64_t incr) {
  ...
  /* #elif defined(__SUNPRO_C) || defined(__SUNPRO_CC) */
  /* Sun CC version have not implemented. */
#else
  uint64_t oldval;
#  if _OPENMP >= 200711
  OMP("omp critical") {
